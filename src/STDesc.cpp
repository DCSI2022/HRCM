...
void growConnectedPlanes(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr &input_cloud,
    ...
    for (auto iter = voxel_map.begin(); iter != voxel_map.end(); ++iter) {
        ...
        if (iter->second->init_octo_) continue;  // visited
        if (!iter->second->plane_ptr_->is_plane_) continue;

        std::vector<VOXEL_LOC> octoCandidateLocs;
        label += 1.;
        octoCandidateLocs.emplace_back(iter->first);

        while (!octoCandidateLocs.empty()){

            VOXEL_LOC curLoc = octoCandidateLocs.back();
            octoCandidateLocs.pop_back();

            auto near = voxel_map.find(curLoc);
            if (near == voxel_map.end()) continue;

            OctoTree *current_octo = voxel_map[curLoc];
            if (current_octo->plane_ptr_->is_plane_ && !current_octo->init_octo_){

                int ptN = current_octo->voxel_points_.size();
                for (int i = 0; i < ptN; ++i) {  // add plane points
                    pcl::PointXYZI pt;
                    pt.x = current_octo->voxel_points_[i](0);
                    pt.y = current_octo->voxel_points_[i](1);
                    pt.z = current_octo->voxel_points_[i](2);
                    pt.intensity = label;
                    cloud->points.emplace_back(pt);
                }
                current_octo->init_octo_ = true;  // checked!

                if (current_octo->connect_[0]){
                    VOXEL_LOC neighbor = iter->first;
                    neighbor.x = neighbor.x + 1;
                    octoCandidateLocs.emplace_back(neighbor);
                }
                if (current_octo->connect_[1]){
                    VOXEL_LOC neighbor = iter->first;
                    neighbor.y = neighbor.y + 1;
                    octoCandidateLocs.emplace_back(neighbor);
                }
                if (current_octo->connect_[2]){
                    VOXEL_LOC neighbor = iter->first;
                    neighbor.z = neighbor.z + 1;
                    octoCandidateLocs.emplace_back(neighbor);
                }
                if (current_octo->connect_[3]){
                    VOXEL_LOC neighbor = iter->first;
                    neighbor.x = neighbor.x - 1;
                    octoCandidateLocs.emplace_back(neighbor);
                }
                if (current_octo->connect_[4]){
                    VOXEL_LOC neighbor = iter->first;
                    neighbor.y = neighbor.y - 1;
                    octoCandidateLocs.emplace_back(neighbor);
                }
                if (current_octo->connect_[5]){
                    VOXEL_LOC neighbor = iter->first;
                    neighbor.z = neighbor.z - 1;
                    octoCandidateLocs.emplace_back(neighbor);
                }
    ...
    if (!cloud->empty()){
        cout << "[ Debug ] growedPlanes : " << cloud->points.size() << endl;
        pcl::io::savePCDFileBinaryCompressed(
                "/home/cyz/workspace/dockerMounted/dockerCodes/STD-master/livox_dataset/growedPlanes.pcd", *cloud);
        pcl::io::savePCDFileBinaryCompressed(
                "/home/cyz/workspace/dockerMounted/dockerCodes/STD-master/livox_dataset/oriCloud.pcd", *input_cloud);
//        int a; cin >> a; // pause
    }

}

void growConnectedPlanes2(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr &input_cloud,
                          std::unordered_map<VOXEL_LOC, OctoTree *> &voxel_map){
    ...
    for (auto iter = voxel_map.begin(); iter != voxel_map.end(); ++iter) {
        ...
        std::vector<OctoTree* > octoCandidateLocs;
        label += 1.;
        octoCandidateLocs.emplace_back(iter->second);

        while (!octoCandidateLocs.empty()){

            OctoTree* current_octo = octoCandidateLocs.back();
            octoCandidateLocs.pop_back();
            ...
            if (current_octo->plane_ptr_->is_plane_ && !current_octo->init_octo_){
                ...
                for (int i = 0; i < 6; ++i)
                    if (current_octo->connect_[i])
                        octoCandidateLocs.emplace_back(current_octo->connect_tree_[i]);
            }
        }
    }
    if (!cloud->empty()){
...
//    std::cout << "[Description] planes size : " << plane_cloud->size() << std::endl;
    ...
    /// TODO test
    growConnectedPlanes2(plane_cloud, voxel_map);
...
void STDescManager::SearchLoop(const std::vector<STDesc> &stds_vec, std::pair<int, double> &loop_result,
    ...
    double best_score = 0, avg_score = 0, sum = 0;
    ...
    for (size_t i = 0; i < candidate_matcher_vec.size(); i++) {
        ...
        if (verify_score > 0) {
            std::cout << ", frame  " << candidate_matcher_vec[i].match_id_.second << std::endl;
            scores.emplace_back(verify_score);
            sum += 1.;
        }
    ...
    if (best_score > 0){
        ...
        std::cout << "[ Debug ] match size : " << sum << ", next score " << scores[(int)(sum*0.9)] << endl;

        fstream fout; fout.open("/home/cyz/workspace/dockerMounted/dockerCodes/STD-master/livox_dataset/scores.txt", ios::out|ios::app);
        for (int i = 0; i < (int)sum; ++i)
            fout << scores[i] << " ";
        fout << endl;
        fout.close();
        std::cout << "[ Debug ] best ID " << best_candidate_id << ", best_score: " << best_score << endl;
    }
                ...
                if (near == voxel_map.end()) {
                    ...
                    if (!current_octo->is_check_connect_[i]) {
                        ...
                        if (near_octo->plane_ptr_->is_plane_) {
                            ...
                            Eigen::Vector3d normal_add = near_octo->plane_ptr_->normal_ ;
                            double theta = acos(normal_diff.dot(normal_add) ) * 180.0 / M_PI;
//                            cout << "[Debug] normal angle : " << theta << endl;
//
//                            Eigen::Vector3d normal_diff = current_octo->plane_ptr_->normal_ -
...
//                            normal_add.norm() < config_setting_.plane_merge_normal_thre_) {  // same PLANE
                            if(theta < config_setting_.plane_merge_normal_thre_ || isnan(theta) ||
                               (180. - theta) < config_setting_.plane_merge_normal_thre_){ // same PLANE
...
//                                cout << " [ Debug ] connected !" << endl;
                            ...
                            current_octo->connect_[i] = false;
                            near_octo->connect_[j] = true;  // boundary
                            near_octo->connect_tree_[j] = current_octo;
                        }
                    }
                }
...
void STDescManager::corner_extractor(
        std::unordered_map<VOXEL_LOC, OctoTree *> &voxel_map,
        const pcl::PointCloud<pcl::PointXYZI>::Ptr &input_cloud,  // Fixme: not used?
        pcl::PointCloud<pcl::PointXYZINormal>::Ptr &corner_points) {
                ...
                for (size_t j = 0; j < data_base_[position].size(); j++) {
                    ...
                    if ((src_std.frame_id_ - data_base_[position][j].frame_id_) >
                        ...
                        if (dis < dis_threshold) {
                            dis_match_cnt++;
                            // rough filter with vertex attached info TODO: gradient check?
...
#pragma omp parallel for
    ...
    if (max_vote >= 4) {
        ...
        std::cout << "[ debug ] max_vote: " << max_vote ;
    } else
        verify_score = -1;
    ...
    for (size_t i = 0; i < source_cloud->size(); i++) {
        ...
        if (kd_tree->nearestKSearch(use_search_point, K, pointIdxNKNSearch,
            ...
            for (size_t j = 0; j < K; j++) {
                ...
                double theta = acos(ni.dot(tni)) * 180. / M_PI;
//                if ((normal_inc.norm() < normal_threshold ||
//                     normal_add.norm() < normal_threshold) &&  // FIXME 20241129
                if ((theta < config_setting_.plane_merge_normal_thre_ || isnan(theta) ||
                     (180. - theta) < config_setting_.plane_merge_normal_thre_) &&
    ...
    std::cout << "[ plane_geometric_verify ] useful match: " << useful_match
...
void STDescManager::PlaneGeomrtricIcp(
    ...
    std::cout << "[ PlaneGeomrtricIcp ] useful match for icp:" << useful_match << std::endl;
...
void OctoTree::init_plane() {
    ...
    if (evalsReal(evalsMin) < config_setting_.plane_detection_thre_ &&
        evalsReal(evalsMin) / evalsReal.sum() < 0.05 &&  //TODO
        plane_ptr_->points_size_ > (config_setting_.voxel_size_/config_setting_.ds_size_) *
                                   (config_setting_.voxel_size_/config_setting_.ds_size_)) {
