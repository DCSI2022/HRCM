...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        for (auto pv : current_cloud->points)
            temp_cloud->points.push_back(pv);  // congregate
    ...
    for (size_t i = 0; i < results.size(); i++) {
        ...
        /**************** save pose ****************/
        string all_poses_dir = "/home/cyz/下载/dockerCodes/STD-master/livox_dataset/opt_poses_std.txt";
        ofstream ofs_pose(all_poses_dir, ofstream::app);
        ofs_pose << to_string(i) << " " << opt_translation[0] << " "
                 << opt_translation[1] << " " << opt_translation[2]
                 << " " << opt_q.x() << " " << opt_q.y()
                 << " " << opt_q.z() << " " << opt_q.w() << endl;
        ofs_pose.close();
    }
