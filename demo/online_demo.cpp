...
bool syncPackages(PointCloud::Ptr &cloud, Eigen::Affine3d &pose) {
    ...
    if (abs(odom_timestamp - laser_timestamp) < 1e-3) {
        ...
        pcl::io::savePCDFileBinaryCompressed(
                "/home/cyz/workspace/dockerMounted/dockerCodes/STD-master/livox_dataset/cloud_registered.pcd", *cloud);
...
//    std::cout << "Recv " << laser_buffer.size() << " , " << odom_buffer.size() << std::endl;
...
void visualizeLoopClosure(const ros::Publisher &publisher,
...
//            "/cloud_registered_body", 100, laserCloudHandler);
            "/cloud_registered", 100, laserCloudHandler);  // cloud in world --- for pointLIO
    ros::Subscriber subOdom =
            nh.subscribe<nav_msgs::Odometry>("/Odometry", 100, OdomHandler);
//            nh.subscribe<nav_msgs::Odometry>("/aft_mapped_to_init", 100, OdomHandler); // for pointLIO
    ...
    PointCloud::Ptr key_cloud_half(new PointCloud);
    int accuFraN = 0;
...
//            pcl::transformPointCloud(*current_cloud_body, *current_cloud_world, pose);
            pcl::copyPointCloud(*current_cloud_body, *current_cloud_world);  // for pointLIO
            ...
            accuFraN ++;
            // Sliding window test
            if (accuFraN > 0.5*config_setting.sub_frame_num_)
                *key_cloud_half += *current_cloud_world;
...
//            if (cloudInd % config_setting.sub_frame_num_ == 0 && cloudInd != 0) {
            if (accuFraN == config_setting.sub_frame_num_ && cloudInd != 0) {
...
//                                robustLoopNoise));
                ...
                pcl::copyPointCloud(*key_cloud_half, *key_cloud);
                key_cloud_half->clear();
                accuFraN = 0.5*config_setting.sub_frame_num_;
            }
    ...
    // down_sampling_voxel(full_map, 0.05);
     ...
     std::cout << "saving map..." << loopNum << std::endl;
